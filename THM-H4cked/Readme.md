...
solution
scroll through the logs and pay attention to the **info** column, you should and will see that the attacker tries multiple diff ip and most  of them fail 
...
you can scan the machine with nmap, you "doing it by the book" but there is no reason for that as we know the vuln port is open and is FTP so we will be going straight to hydra

in a termial we will run 


hydra -l jenny -P /usr/share/wordlists/rockyou.txt ftp://10.130.137.117
or what is in he image the important thing is where the wordlist is 
ps please extract the file, run this command
 ```
 gzip -d rockyou.txt.gz
 ```
 now we have the password we will login and to do  that we will run
...
this downloads the shell file to your kali
...
with this acces has been grante all you have to do s to follow the exact commands  the attacker did gotten from the packets which will lead to the last flag
